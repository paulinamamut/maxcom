...
<head>
    ...
    <div style=""background: none repeat scroll 0 0 #c32030; color: #fff;"">
        ...
        public ActionResult ContactoComercial(string nombre, string puesto, string empresa, string email, string empleados,
            ...
            var qs = "";
            var request = WebRequest.Create(string.Format("https://www.google.com/recaptcha/api/siteverify?secret=6LfumgcTAAAAAG4_s8P4MPcePXs8Oaoi_QItaIT6&response={0}", Request.Form["g-recaptcha-response"]));
            request.Method = "POST";
            ...
            using (var response = request.GetResponse()) {
                ...
                using (var responseDataStream = response.GetResponseStream()) {
                    if (responseDataStream == null) {
                        ViewData["Exception"] = "<img src=\"/img/error.png\"/>Por favor completa el Captcha.";
                    } else {
            ...
            ViewData["Success"] = SendEmail(msg, "Contacto Comercial Maxcom");
            if ((string)ViewData["Success"] == "OK")
                return Redirect("/Gracias");
            return View();
        }

        private static string SendEmail(string message, string subject) {
            try {
